...
void acciones_basicas(){
    ...
    *elemento = 8;
    ...
    verificar(arbol_guardar(arbol, NULL) == EXITO, "Se puede insertar NULL");
    verificar(arbol_guardar(arbol, "0",  &elemento[0]) == EXITO, "Inserto un elemento con exito");
    verificar(arbol_borrar(arbol, "0",  &elemento[0]) == EXITO, "Borro un elemento con exito");
...
void inserciones_y_busquedas(){
    ...
    {
        if(arbol_guardar(arbol, elementos[acciones]) == EXITO){
    ...
    while ((acciones < TOPE) && (seguimos == true))
    {
        if(*(unsigned char*)arbol_obtener(arbol, elementos[acciones]) == *(unsigned char*)elementos[acciones]) acciones++;
        else seguimos = false;
    ...
    *non = 42;
    verificar((arbol_obtener(arbol, non) == NULL), "Buscar un elemento no insertado devuelve NULL");
    free(non);
    arbol_destruir(arbol);
    free(elementos);
...
abb_t* cargar_arbol(unsigned char** elementos){
    ...
    while ((acciones < tope))
    {
        arbol_guardar(arbol, elementos[acciones]);
...
void pruebas_de_borrado(){
    ...
    verificar((*(unsigned char*)arbol_obtener(arbol, &valores[6]) == valores[6]), "Encontramos el elemento repetido ");
    verificar((arbol_borrar(arbol, &valores[6]) == EXITO), "Borramos con exito ese elemento");
    verificar((arbol_obtener(arbol, &valores[6]) == NULL), " No encontramos el elemento repetido ");
    ...
    verificar((*(unsigned char*)arbol_raiz(arbol) == valores[8]), "La raiz es correcta ");
    verificar((*(unsigned char*)arbol_obtener(arbol, &valores[8]) == valores[8]), "Encontramos el elemento repetido ");
    ...
    verificar((arbol_obtener(arbol, &valores[8]) == NULL), "La raiz original ya no se encuentra en el arbol");
    arbol_destruir(arbol);
    free(elementos);
}
