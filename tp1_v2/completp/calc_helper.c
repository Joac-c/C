...
bool calc_parse(const char *tok, struct calc_token *parsed) {
    ...
    if (strlen(tok) == 1) {
        ...
        if (op == '+') {
            ...
            parsed->oper.precedencia = ASSOC_LEFT;
            parsed->oper.asociatividad = 1;
        }
        else if (op == '-') {
            ...
            parsed->oper.precedencia = ASSOC_RIGHT;
            parsed->oper.asociatividad = 3;
        }
        else if (op == '?') {
